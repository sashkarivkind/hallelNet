...
m_lump_vec=4;%1:5; %m_lump nodes are lumped into a hub. m_lump_vec is an array of values m_lump assumes. to lump into the hub
...
for qq=1:length(m_lump_vec)
    ...
    %W_dehub=W;
    %W_dehub(1:m_lump,1:m_lump)=0;
    %o=sim_dyn(W_dehub);
    %o.m_dehub=m_lump;
    %results{ll}.closed_loop_dehub_sims{qq}=o;
end
results{ll}.original_net=struct('gamma_k_out',gamma_k_out,'k_m_out',k_m_out,...
    'mean_k', mean_k,'gamma_fit',gamma_fit);

%simulate
results{ll}.sim = sim_dyn(W);
results{ll}.upnet_sim=sim_dyn(diag(sign(sum(W,2)))*W); %this is a network with a guaranteed fixed point at s=(1,1,1,1,1,1,1....1)
